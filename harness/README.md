...
Two readings recorded on 2026-08-10 so they are not re-derived. `litsearch-collision` ("Is my idea already published somewhere?") fails on every epoch, and the event stream shows why: four proposals are staged, so the agent asks which one "my idea" refers to and never selects at all. That is a case-design artifact, not a description defect, and it is left in place rather than reworded mid-comparison — a dataset edited between two sweeps invalidates the comparison it was meant to support. `litsearch-oblique` is flaky: it failed in the 2026-08-10 sweep and passed on re-measurement, so a single epoch on that case settles nothing.
