...
- **A fan-out is invisible on every path.** The Inspect agent holds only `bash` and `text_editor`, and the dev runner captures the child's stdout only, so neither can see a host spawning helper agents. The execution-shape sections in review, supervise, check and write (2026-09-02, after a real supervise run under a workflow-by-default host mode reached 27 agents and ~8.3M tokens) are guarded by L0 pins alone, and their effect is measurable only on a real host run with such a mode on. The cheap follow-up is a dev-runner cost probe: the stream-json result event already carries `total_cost_usd` and `num_turns`, and `routing.py` already parses `tool_use` events.
