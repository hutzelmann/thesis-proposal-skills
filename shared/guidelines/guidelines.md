...
4. **Methodology for Research: \<Methodology\>** — One methodology from the closed set, with its required subsections per `structure.json`. Never combine methodologies — decide on one and stick to it. The set shipped here (Prototype Implementation, Theoretical Analysis, Systematic Literature Review, User Study, Controlled Experiment) is a default, not a claim about which methodologies exist: a workspace `guidelines.md` may add branches, replace them, or disable ones its program does not accept, and the set that applies is always the one in `structure.json` after those overrides. What is not negotiable is the closure — whatever the set contains, a proposal declares exactly one member of it.
