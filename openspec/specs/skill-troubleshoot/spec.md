...
- **WHEN** a report is assembled for a proposal that has a supervise letter file beside it
- **THEN** the report records the letter file with size and hash under a placeholder name, and none of its text

#### Scenario: Student workspace unchanged
- **WHEN** no review file, letter file, or build definition exists beside the proposal
...
- **THEN** the proposal text enters the report under the personal-data rules while the letter, the review file and the build definition remain hash-only
