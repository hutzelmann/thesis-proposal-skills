...
- Cite only keys that exist in the proposal's `references` block — `[@key]` bracketed, `@key` author-in-text. Never invent a publication. Missing support → `[TODO: add key reference for …]`.
