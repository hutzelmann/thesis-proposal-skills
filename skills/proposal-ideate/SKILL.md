...
- **The student brings another thesis's future work** — usually the whole finished thesis as a PDF: read only its closing chapters. Locate Future Work, Limitations, and Conclusion from the table of contents or by scanning headings, read those pages, say which ones you read, and leave the rest unread — a thesis read end to end turns you into the person with the ideas, which this skill must never become. If you cannot ingest the PDF here, ask for those sections as text, say why only that slice, and proceed identically. Then treat what you found like a supervisor's topic list above, with three differences. Nobody vetted a future-work list and it was written under submission pressure, so the first move is whether an item carries a thesis or a paragraph — "which of these needs a method, and which is just more of the same data?". The source thesis is the closest prior work the new proposal must state a delta against, so ground the chosen item early: an item suggested two years ago is often already done. And a focus that reads as the previous thesis's leftover tasks is a scope, not a research focus — press until the question is the student's own.
...
Create `<slug>.md` in the workspace's proposal location — the working directory, unless the workspace `guidelines.md` sets `[paths] proposals` to a subdirectory (slug from the working title; never overwrite an existing proposal without asking) — and rename the notes file to the same slug if the provisional one diverged; the notes file lives beside the proposal. This is also where you confirm, once, the exact start and submission months — pre-filled from the preamble's months estimate.

- Body: the working title as the file's leading `# ` line (its only H1), the subtitle as an emphasized `*…*` paragraph beneath it, then the problem sketch in a few sentences, why it matters in one or two, candidate research-question directions as a bullet list (marked as candidates — or as final when the student called them final), what drew interest from the literature, the timeframe as a plain note, and `[TODO: …]` markers for submission-blocking gaps only — everything else belongs in the notes file, and the closing references heading sits above the metadata block. The seed is a sketch, not a finished proposal — record the timeframe as a note, never as a `## Timeline` section.
- The subtitle wording carries the language and the degree level, both from the preamble: `*Bachelor's Thesis Proposal*` / `*Master's Thesis Proposal*` in English and `*Exposé zur Bachelorarbeit*` / `*Exposé zur Masterarbeit*` in German, with `*[TODO: state the degree level]*` only when the level never came up. Later tooling infers the language from that wording — there is no `lang` key.
- Trailing metadata block (blank line before `---`): `references:` with any starter entries found during grounding (CSL-YAML) — write `references: []` when none were found; the key must always be present, and it is the block's only key. Never write an `author` key or a TODO for one: proposals are anonymous.
- When the session started from another finished thesis, record where it came from: a starter `references` entry when that thesis is publicly retrievable, and a line in the notes file alone when it is not. Never invent publication metadata to make an unpublished thesis citable.
