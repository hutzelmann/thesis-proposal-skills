...
- ~~**S3 — typst template fidelity**~~ **Resolved (spike 2026-07-29, see `openspec/changes/archive/2026-07-29-s3-typst-template-spike/design.md`):** compact layout fully reproduced in `skills/proposal-publish/templates/proposal.typ`; RQ styling solved markdown-natively via `rq-filter.lua` (plain ordered list → bold `RQ n:` blocks, typst output only). Follow-ups: compact numeric `.csl` (step 4) and bibliography heading styling.
